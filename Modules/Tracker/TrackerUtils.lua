...
local function _GetZoneName(zoneOrSort, questId)
    -- Always return a non-nil zone label to keep tracker stable
    if zoneOrSort == nil then
        Questie:Debug(Questie.DEBUG_CRITICAL, "[TrackerUtils:_GetZoneName] zoneOrSort is nil for quest", questId, "- using 'Unknown Zone'")
        return "Unknown Zone"
    end
    
    -- Special handling for runtime stubs with custom zone names
    if zoneOrSort == -9999 then
        Questie:Debug(Questie.DEBUG_INFO, "[TrackerUtils:_GetZoneName] Detected runtime stub special zoneOrSort (-9999) for quest", questId)
        local QuestiePlayer = QuestieLoader:ImportModule("QuestiePlayer")
        if QuestiePlayer and QuestiePlayer.currentQuestlog then
            local quest = QuestiePlayer.currentQuestlog[questId]
            if quest then
                Questie:Debug(Questie.DEBUG_INFO, "[TrackerUtils:_GetZoneName] Found quest in currentQuestlog, __isRuntimeStub:", tostring(quest.__isRuntimeStub), "runtimeZoneName:", tostring(quest.runtimeZoneName))
                if quest.__isRuntimeStub and quest.runtimeZoneName then
                    Questie:Debug(Questie.DEBUG_INFO, "[TrackerUtils:_GetZoneName] Using runtime zone name for quest", questId, ":", quest.runtimeZoneName)
                    return quest.runtimeZoneName
                end
            else
                Questie:Debug(Questie.DEBUG_INFO, "[TrackerUtils:_GetZoneName] Quest", questId, "not found in currentQuestlog")
            end
        else
            Questie:Debug(Questie.DEBUG_INFO, "[TrackerUtils:_GetZoneName] QuestiePlayer or currentQuestlog not available")
        end
    end
    ...
    if sortObj == "byZone" or sortObj == "byZonePlayerProximity" or sortObj == "byZonePlayerProximityReversed" then
        ...
        elseif (zoneOrSort) < 0 then
            ...
            Questie:Debug(Questie.DEBUG_CRITICAL, "[TrackerUtils:_GetZoneName] zoneOrSort", zoneOrSort, "of quest", questId, "is not in the Database! Using zone label:", zoneName)
        end
    else
    ...
    for questId, quest in pairs(QuestiePlayer.currentQuestlog) do
        if quest then
            ...
            if quest:IsComplete() == 1 or (not next(quest.Objectives)) then
                ...
                local count = 0
                for _, Objective in pairs(quest.Objectives) do
                    -- Guard against divide-by-zero and missing fields; use Completed flag as fallback
                    local collected = tonumber(Objective.Collected) or 0
                    local needed = tonumber(Objective.Needed)
                    local p
                    if needed and needed > 0 then
                        p = math.min(1, collected / needed)
                    else
                        -- If Blizzard marks it finished or our Completed flag is true, treat as done
