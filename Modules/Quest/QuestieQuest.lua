...
-- Create a minimal runtime Quest object for quests missing from the compiled DB,
-- using the live quest log cache as source of truth for title/objectives/progress.
local function _CreateRuntimeQuestStub(questId, questLogData)
    ---@type Quest
    -- Derive title/isComplete/objectives from QuestLogCache when available;
    -- otherwise fall back to Blizzard API to avoid cache timing issues.
    local title = questLogData and questLogData.title
    local isCompleteFlag = questLogData and questLogData.isComplete
    local rawObjectives = questLogData and questLogData.objectives
    local questLevel = nil

    if (not title) or (isCompleteFlag == nil) or (not rawObjectives) or (not questLevel) then
        local qli = GetQuestLogIndexByID and GetQuestLogIndexByID(questId)
        if qli then
            local t, level, questTag, isHeader, _, apiIsComplete = GetQuestLogTitle(qli)
            if (not isHeader) then
                title = title or t
                questLevel = questLevel or level
                isCompleteFlag = (isCompleteFlag ~= nil) and isCompleteFlag or apiIsComplete
            ...
            Questie:Debug(Questie.DEBUG_INFO, "[_CreateRuntimeQuestStub] Quest", questId, "not found in Blizzard quest log")
        end
    end
    ...
    if currentZoneName then
        Questie:Debug(Questie.DEBUG_INFO, "[_CreateRuntimeQuestStub] Quest", questId, "will use zone:", currentZoneName)
    else
    ...
    -- Method to mirror Quest object API
    function stub:IsComplete()
        -- For runtime stubs, check the isComplete flag first, then fall back to QuestieDB
    ...
    -- Build basic objective rows from quest log cache so the tracker can show progress.
    local objectivesSrc = rawObjectives
    if objectivesSrc then
        Questie:Debug(Questie.DEBUG_INFO, "[_CreateRuntimeQuestStub] Creating", #objectivesSrc, "objectives for quest", questId)
        for i, obj in ipairs(objectivesSrc) do
            local desc = obj.text or (obj.raw_text) or (obj.description) or ("Objective " .. i)
            ...
            stub.Objectives[i] = {
                ...
                Completed = (finished == true) or ((tonumber(numFulfilled) or 0) > 0 and (tonumber(numRequired) or 0) > 0 and (tonumber(numFulfilled) == tonumber(numRequired))),
                Coordinates = nil,
                RequiredRepValue = nil,
            }
            -- Do not assign Update here to avoid forcing QuestLogCache dependency until it’s ready
            -- stub.Objectives[i].Update = _QuestieQuest.ObjectiveUpdate
        end
    else
        Questie:Debug(Questie.DEBUG_INFO, "[_CreateRuntimeQuestStub] No objectives found for quest", questId)
    end

    -- Test quest tag functionality for runtime stubs
    local testQuestType, testQuestTag = QuestieDB.GetQuestTagInfo(questId)
    if testQuestType or testQuestTag then
        Questie:Debug(Questie.DEBUG_INFO, "[_CreateRuntimeQuestStub] Quest", questId, "has questType:", testQuestType, "questTag:", testQuestTag)
    end

    return stub
end

-- Function to update runtime stubs with fresh objective data from quest log
local function _UpdateRuntimeQuestStub(questId, stub)
    if not stub or not stub.__isRuntimeStub then
        return false
    end
    ...
    local title, level, questTag, isHeader, _, apiIsComplete = GetQuestLogTitle(qli)
    if isHeader or not title then
        return false
    end
    
    -- Update completion status and quest level
    stub.isComplete = (apiIsComplete == 1) or nil
    if level then
        stub.Level = level
        stub.level = level
    end
    
    -- Update objectives using 3.3.5a API
    local newObjectives = {}
    ...
    if foundObjectives then
        stub.Objectives = newObjectives
        Questie:Debug(Questie.DEBUG_INFO, "[_UpdateRuntimeQuestStub] Updated", #newObjectives, "objectives for quest", questId)
        return true
    end
    ...
    for questId, data in pairs(QuestLogCache.questLog_DO_NOT_MODIFY) do -- DO NOT MODIFY THE RETURNED TABLE
        if (not QuestieDB.QuestPointers[questId]) then
            ...
            if quest then
                ...
                QuestiePlayer.currentQuestlog[questId] = questId -- Should not typically happen
            end
...
function _QuestieQuest.ObjectiveUpdate(self)
    ...
    -- For runtime stubs, try to get fresh data directly from quest log API
    local quest = QuestiePlayer.currentQuestlog[self.questId]
    if quest and quest.__isRuntimeStub then
        ...
        if qli and SelectQuestLogEntry and GetNumQuestLeaderBoards and GetQuestLogLeaderBoard then
            ...
            if numObjectives and self.Index <= numObjectives then
                local description, type, finished, numFulfilled, numRequired = GetQuestLogLeaderBoard(self.Index)
                if description then
                    -- Format description with progress if needed
                    local formattedDesc = description
                    if numRequired and tonumber(numRequired) > 1 and not string.find(description, "%d+/%d+") then
                        formattedDesc = description:gsub(":?%s*$", "") .. ": " .. (numFulfilled or 0) .. "/" .. numRequired
                    end
                    
                    self.Description = formattedDesc
                    self.Collected = tonumber(numFulfilled) or 0
    ...
    -- Fall back to regular quest log cache for non-runtime stubs
...
-- Export runtime stub creation function for use by event handlers
QuestieQuest._CreateRuntimeQuestStub = _CreateRuntimeQuestStub
